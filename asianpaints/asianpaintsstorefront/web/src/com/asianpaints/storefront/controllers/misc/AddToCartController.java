 ...
 *
 */
package com.asianpaints.storefront.controllers.misc;
...
import com.asianpaints.core.cart.facade.DefaultAsianpaintsCartFacade;
import com.asianpaints.storefront.controllers.ControllerConstants;
import com.asianpaints.storefront.forms.AsianpaintsAddToCartForm;
...
public class AddToCartController extends AbstractController
{
	...
	private static final String SHOWN_PRODUCT_COUNT = "asianpaintsstorefront.storefront.minicart.shownProductCount";
	...
	@Resource(name = "defaultAsianpaintsCartFacade")
	private DefaultAsianpaintsCartFacade defaultAsianpaintsCartFacade;
	...
	public String addToCart(@RequestParam("productCodePost") final String code, final Model model,
			@Valid final AsianpaintsAddToCartForm form, final BindingResult bindingErrors)
	{
		...
		/*
		 * final String selectedAddons = form.getAddons(); final String[] addonsList = addons.split(","); final
		 * List<String> addonsListAL = Arrays.asList(addonsList);
		 */
		final List<String> addons = form.getAddons();
		...
		else
		{
			try
			{
				final CartModificationData cartModification = defaultAsianpaintsCartFacade.addToCart(code, qty, addons);
				model.addAttribute("quantity", Long.valueOf(cartModification.getQuantityAdded()));
				model.addAttribute("entry", cartModification.getEntry());
				model.addAttribute("cartCode", cartModification.getCartCode());
				//model.addAttribute("addonsEntry", addons);
				model.addAttribute("selectedAddons", addons);
