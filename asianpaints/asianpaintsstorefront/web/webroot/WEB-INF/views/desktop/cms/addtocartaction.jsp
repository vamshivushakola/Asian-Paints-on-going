...
<form:form method="post" id="asianpaintsAddToCartForm" class="add_to_cart_form span-5" action="${addToCartUrl}">

<input type="hidden" id="addons" name="addons" class="addons" value="">
	...
	<c:if test="${empty showAddToCart ? true : showAddToCart}">
		...
		<c:choose>
			...
			<c:otherwise>
				<button id="addToCartButton" type="${buttonType}" onclick="myFunction()"  class="addToCartButton" disabled="disabled">
...
<script>
function myFunction() {
	var checkedAddonVals = [];
	$('#addonsDiv :checked').each(function() {
		checkedAddonVals.push($(this).val());
		
	});
	//alert(checkedAddonVals);
    document.getElementById("addons").value =  checkedAddonVals; 
}
</script>
