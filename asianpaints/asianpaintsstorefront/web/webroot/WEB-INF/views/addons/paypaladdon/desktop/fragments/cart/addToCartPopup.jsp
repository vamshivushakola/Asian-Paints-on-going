...
<div>
	...
	<c:if test="${not empty payPalCreditShortcut and not empty payPalExpressCheckoutShortcut}">
		<div class="orPadding"><spring:theme code="popup.cart.or"/></div>
	</c:if>
	<c:if test="${not empty payPalCreditShortcut}">
		<cms:component component="${payPalCreditShortcut}" evaluateRestriction="true"  />
	</c:if>
	</div>
</div>
